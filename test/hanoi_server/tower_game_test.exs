defmodule TowerGameTest do
  ...
  test "does server create board correctly" do
    ...
    output = Hanoi.TowerGame.get_board_state(name)
    assert output == expected_state

    output_string = Hanoi.TowerGame.get_board_state_as_text(name)
  ...
  test "does server move stone correctly" do
    ...
    before = Hanoi.TowerGame.get_board_state_as_text(name)
    ...
    after_s = Hanoi.TowerGame.get_board_state_as_text(name)
  ...
  test "does server run get moves correctly" do
    ...
    assert Hanoi.TowerGame.get_moves_to_win(name) == expected 
  end
  ...
  test "does server reset board correctly" do
    ...
    assert Hanoi.TowerGame.get_board_state(name) == expected_state
    assert Hanoi.TowerGame.get_number_moves(name) == 1
    assert Hanoi.TowerGame.get_number_stones(name) == 3

    Hanoi.TowerGame.reset_game(name, 4)
 
    reset_state = %Hanoi.Board{left: [1, 2, 3, 4], centre: [], right: []}
    assert Hanoi.TowerGame.get_board_state(name) == reset_state
  ...
  test "Does server pass complete test correctly" do
    name = :complete_pass 
    Hanoi.TowerGame.add_game(name, 3)
    moves = Hanoi.TowerGame.get_moves_to_win(name) 
